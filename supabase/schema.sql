...
create policy "Admins see all profiles; everyone sees their own" on profiles for
select using (
    my_role() = 'admin'
    or id = auth.uid()
  );
...
create policy "Admin or own department can insert appraisals" on appraisals for
insert with check (
    my_role() = 'admin'
    or my_department() = department
  );
drop policy if exists "Admin or own department can update appraisals" on appraisals;
create policy "Admin or own department can update appraisals" on appraisals for
update using (
    my_role() = 'admin'
    or my_department() = department
  );
drop policy if exists "Admin or own department can delete appraisals" on appraisals;
create policy "Admin or own department can delete appraisals" on appraisals for delete using (
  my_role() = 'admin'
  or my_department() = department
);
-- ============ INVESTMENTS ============
...
create table if not exists investments (
  ...
  lga text,
  amount numeric,
  source text check (source in ('FDI', 'DDI', 'HYBRID')),
...
create policy "Everyone can view investments" on investments for
select using (auth.role() = 'authenticated');
drop policy if exists "Only admin can insert investments" on investments;
create policy "Only admin can insert investments" on investments for
insert with check (my_role() = 'admin');
drop policy if exists "Only admin can update investments" on investments;
create policy "Only admin can update investments" on investments for
update using (my_role() = 'admin');
drop policy if exists "Only admin can delete investments" on investments;
create policy "Only admin can delete investments" on investments for delete using (my_role() = 'admin');
...
create table if not exists employees (
  ...
  -- null = agency-wide/executive
  gender text check (gender in ('Male', 'Female')),
  job_category text check (
    job_category in ('Contract', 'Fulltime', 'Pool', 'IT', 'NYSC')
  ),
  designation text,
...
create policy "Everyone can view employees" on employees for
select using (auth.role() = 'authenticated');
drop policy if exists "Only admin can insert employees" on employees;
create policy "Only admin can insert employees" on employees for
insert with check (my_role() = 'admin');
drop policy if exists "Only admin can update employees" on employees;
create policy "Only admin can update employees" on employees for
update using (my_role() = 'admin');
drop policy if exists "Only admin can delete employees" on employees;
create policy "Only admin can delete employees" on employees for delete using (my_role() = 'admin');
...
create policy "Everyone can view custom KPIs" on kpi_definitions for
select using (auth.role() = 'authenticated');
drop policy if exists "Only admin can create custom KPIs" on kpi_definitions;
create policy "Only admin can create custom KPIs" on kpi_definitions for
insert with check (my_role() = 'admin');
drop policy if exists "Only admin can update custom KPIs" on kpi_definitions;
create policy "Only admin can update custom KPIs" on kpi_definitions for
update using (my_role() = 'admin');
drop policy if exists "Only admin can delete custom KPIs" on kpi_definitions;
create policy "Only admin can delete custom KPIs" on kpi_definitions for delete using (my_role() = 'admin');
-- ============ REALTIME ============
